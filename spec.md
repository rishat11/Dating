...
- Ввести **очередь** (Redis Streams / RabbitMQ / Kafka и т.п.) для:
  ...
  - отправки уведомлений (мягкое напоминание о паузе в переписке, рост уровня);
...
- **Обязательно**: имя/ник, возраст, пол, кого ищет (М/Ж/все), город или «не указывать». **Фото**: желательно одно фото; при регистрации без фото анкета допускается с ограничением видимости в ленте (показываться реже или с пометкой «без фото») и напоминанием позже добавить фото.
...
- Ограничение количества лайков в день: **100 бесплатно**; расширение сверх лимита или снятие лимита — за донат/подписку.

### 3.4 Чаты

- Личный чат только между парой (после матча). Сообщения идут через бота: пользователь пишет боту в контексте «чат с пользователем X», бот пересылает сообщение партнёру. **Обмен контактами** (номер, username, соцсети) разрешён с самого начала: в чате доступна кнопка «Обменяться контактами»; при взаимном нажатии бот раскрывает контактные данные обоих. До обмена контакты через бота не раскрываются.
...
- В шапке/описании чата: прогресс-бар и процент, например:  
  ...
  Подсказки («+3% за обсуждение музыки», «до 50% осталось N сообщений») показывать **редко**: при застревании на уровне или раз в N сообщений, чтобы не превращать диалог в гонку за процентами.
- Служебное сообщение о росте индекса — **только при смене уровня** (например, переход 30% → 31%, 50% → 51%); при обычном приросте в рамках уровня — обновление только прогресс-бара в шапке, без сообщения в ленте чата (чтобы не засорять переписку).
...
| **2. Поведенческий** | Длина сообщений, скорость ответов, голосовые | Быстрые ответы (<2 мин) — опциональный бонус раз в час +1%; длительная пауза (>24–48 ч) — мягкое снижение или только пауза роста индекса (не жёсткий штраф); односложные ответы не дают роста, но не понижают индекс; сообщения >100 символов — микро-бонус; первое голосовое +5%; голосовое >1 мин +2%. |
...
- **Напоминание о паузе**: при длительном отсутствии партнёра (например, >24–48 ч) и лёгком снижении индекса — одно нейтральное уведомление («Индекс немного снизился из‑за паузы. Когда будет время — напиши пару слов»), без давления; опция отключения таких напоминаний в настройках.
- **Конфликт/негатив**: заморозка индекса — в первую очередь по жалобе второго пользователя или при явном детекте оскорблений (модерация). При автоматическом срабатывании по тону — короткая пауза роста (например, 1 ч) без морализующего сообщения («индекс заморожен за негатив» не показывать). Повторные инциденты — усиление мер.
...
- **Базовая**: лимит 100 лайков в день бесплатно; разблокировка дополнительных лайков или снятие лимита, «суперлайки» — за звёзды Telegram / подписку.
- **Индекс совместимости** (из feature.md):
  - **Ускоритель (бустер)**: донат (например, 50 звёзд) — «Бустер совместимости» на 24 ч (рост индекса x2). Рекомендуется применять по согласию обоих в паре или давать эффект обоим при покупке одним, чтобы не создавать ощущения pay-to-win.
  - **Секретный вопрос**: покупка анонимного вопроса от ИИ для диалога; рост индекса — естественный результат обсуждения, а не гарантированный «+N%»; в интерфейсе не подчёркивать платное происхождение роста.
  - **Аналитика (обезличенная)**: платный отчёт о стиле общения пары — темы, тон, длина сообщений, рекомендации для диалога. Не включать оценку чувств одного пользователя к другому («нравишься на X%», «искренность собеседника») — такие формулировки подрывают доверие и не реализуются.
...
- Обмен контактами разрешён с момента создания пары по взаимному согласию (кнопка «Обменяться контактами» в чате); принудительная передача контактов (спам, без согласия второго) запрещена.
...
### 7.1 Обмен контактами и защита от спама

- **Технически**: обмен контактами доступен с самого начала чата через кнопку «Обменяться контактами»; при взаимном нажатии бот передаёт обоим контактные данные (номер, username и т.д.). В остальных случаях сообщения пересылаются через бота без раскрытия контакта.
- **Модерация**: автоматическая проверка на **спам** контактами (массовая рассылка номера/ссылок незнакомым, попытки обойти блокировку). При однократном упоминании номера или соцсети в паре — одно предупреждение («удобнее обменяться контактами через кнопку»), без заморозки индекса. Заморозка или ограничение — только при повторных злоупотреблениях или по жалобе. Жалобы пользователей — ручная проверка модератором.
...
- **ИИ-анализ (обязательное ограничение)**: в качестве API для ИИ разрешён **только DeepSeek API**. Использование других LLM (OpenAI, Anthropic, YandexGPT, локальных моделей и т.д.) не допускается. Все сценарии PRO (анализ диалога, секретный вопрос, обезличенная аналитика стиля общения) реализуются исключительно через DeepSeek API.
...
- При разработке бота: отдельные модули — онбординг (в т.ч. выбор языка ru/en), анкета (в т.ч. **геолокация** — приём координат, обновление, отключение; фото опционально при регистрации с ограничением видимости), матчинг (лента с лимитом **100 лайков в день**, выборка ближайших, расчёт расстояния — Haversine или PostGIS), чат (в т.ч. кнопка **«Обменяться контактами»** с первого дня пары), сервис расчёта индекса (с уровнями 1–4, смягчённые правила по молчанию и конфликту — п. 4.2, 4.4), разблокировки (плейлист, челленджи, облако, голосование, PDF), платежи (Telegram Stars). **Локализация**: все пользовательские тексты (сообщения бота, кнопки, подсказки, уведомления) должны поддерживать русский и английский в зависимости от `locale` пользователя (словари/файлы переводов, выбор строки по locale при формировании ответа).
